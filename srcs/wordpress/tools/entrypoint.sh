...
# confirma criação do diretório wp devido ao bind mount
mkdir -p "${WP_DIR}"
...
if [ ! -f "${WP_DIR}/wp-config.php" ]; then
  ...
  # criar um usuário adicional caso as variáveis estejam definidas
  if [ -n "${EXTRA_USER:-}" ] && [ -n "${EXTRA_USER_EMAIL:-}" ]; then
    if ! ${WP_CLI} user get "${EXTRA_USER}" --path="${WP_DIR}" --allow-root >/dev/null 2>&1; then
      ${WP_CLI} user create "${EXTRA_USER}" "${EXTRA_USER_EMAIL}" --user_pass="${EXTRA_USER_PASSWORD}" --allow-root
    else
...
# exec php-fpm em foreground (PID 1)
exec php-fpm8.2 -F
