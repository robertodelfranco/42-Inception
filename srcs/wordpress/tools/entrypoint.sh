...
if [ ! -f "${WP_DIR}/wp-config.php" ]; then
  ...
  ${WP_CLI} user create user01 user01@${DOMAIN} --user_pass="userpass123" --allow-root
fi
...
#   # baixa wp-cli
#   if [ ! -x "${WP_CLI}" ]; then
#     if ! curl -o "${WP_CLI}" https://raw.githubusercontent.com/wp-cli/builds/gh-pages/phar/wp-cli.phar; then
...
#     if ! ${WP_CLI} user get "${EXTRA_USER}" --allow-root >/dev/null 2>&1; then
